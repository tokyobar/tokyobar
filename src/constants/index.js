import {
    people01,
    people02,
    people03,
    facebook,
    instagram,
    linkedin,
    twitter,
    airbnb,
    binance,
    coinbase,
    dropbox,
    send,
    shield,
    star,
} from '../assets'

export const navLinks = [
    {
        id: 'home',
        title: 'Home',
    },
    {
        id: 'features',
        title: 'Features',
    },
    {
        id: 'product',
        title: 'Product',
    },
    {
        id: 'clients',
        title: 'Clients',
    },
]

export const features = [
    {
        id: 'feature-1',
        icon: star,
        title: 'Ingredients',
        content:
            'We use only genuine ingredients and materials, providing you with a vaping experience that is as real as it gets',
    },
    {
        id: 'feature-2',
        icon: shield,
        title: 'Authenticity',
        content:
            'Each product is verified and crafted with genuine materials for unparalleled satisfaction.',
    },
    {
        id: 'feature-3',
        icon: send,
        title: 'Quality',
        content:
            'Every TokyoBar Vape is certified 100% authentic, offering you peace of mind and exceptional quality.',
    },
]

export const feedback = [
    {
        id: 'feedback-1',
        content:
            'The craftsmanship and attention to detail in TokyoBar Vapes are truly impressive. I’ve never had a better vaping experience.',
        name: 'Leyna maebul',
        title: 'Customer',
        img: people01,
    },
    {
        id: 'feedback-2',
        content:
            'Switching to TokyoBar Vapes was the best decision I made. The quality and authenticity are unparalleled.',
        name: ' Muna Al-Shaerark',
        title: 'Customer',
        img: people02,
    },
    {
        id: 'feedback-3',
        content:
            'I highly recommend TokyoBar Vapes to anyone looking for a premium and authentic vaping product',
        name: 'Rami',
        title: 'Customer',
        img: people03,
    },
]

export const stats = [
    {
        id: 'stats-1',
        title: 'Puffs',
        value: '2000+',
    },
    {
        id: 'stats-2',
        title: 'Trusted Brand',
        value: '100%',
    },
    {
        id: 'stats-3',
        title: 'Users',
        value: '50k+',
    },
]

export const footerLinks = [
    {
        title: 'Useful Links',
        links: [
            {
                name: 'Features',
                link: 'https://www.hoobank.com/features/',
            },
            {
                name: 'Flavours',
                link: 'https://www.hoobank.com/product',
            },
            {
                name: 'Testimonials',
                link: 'https://www.hoobank.com/clients/',
            },
        ],
    },

]

export const socialMedia = [
    {
        id: 'social-media-1',
        icon: instagram,
        link: 'https://www.instagram.com/',
    },
    {
        id: 'social-media-2',
        icon: facebook,
        link: 'https://www.facebook.com/',
    },
    {
        id: 'social-media-3',
        icon: twitter,
        link: 'https://www.twitter.com/',
    },
    {
        id: 'social-media-4',
        icon: linkedin,
        link: 'https://www.linkedin.com/',
    },
]

export const clients = [
    {
        id: 'client-1',
        logo: airbnb,
    },
    {
        id: 'client-2',
        logo: binance,
    },
    {
        id: 'client-3',
        logo: coinbase,
    },
    {
        id: 'client-4',
        logo: dropbox,
    },
]
